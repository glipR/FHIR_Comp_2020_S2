...
def solve(lines):
    ...
    for testno in range(n_pracs):
        ...
        serum = int(serum)
        patients = {
            id: pat for id, pat in data["patients"].items()
            if prac_ids[testno] in pat.practitioners
        }
        for pat in patients:
            pat.platelets = None
        for obs in data["observations"].values():
            if obs.code != "32623-1": continue
            if not obs.patient in patients: continue
            if patients[obs.patient].platelets is None or patients[obs.patient].platelets[1] < parser.parse(obs.effective):
                patients[obs.patient].platelets = (
                    (obs.value["value"], obs.value["unit"]),
                    parser.parse(obs.effective),
                )
        saveable_patients = sorted([
            pat for pat in patients.values()
            if pat.platelets is not None
        ], key=lambda pat: pat.platelets[0][0])

        # Assert units are the same.
        assert len(list(itertools.groupby(saveable_patients, lambda pat: pat.platelets[0][1]))) == 1, "More than one unit of measurement detected."
