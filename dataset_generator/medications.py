...
def generate_medications(condition_list, max_number_medications, api_url, api_token, generate_conditions, directory_path):
    ...
    for cond_key, cond_display, cond_count, medications in (
        condition_list
    ):
        for key, display, count in (
            ...
            while items_left > 0:
                request_url = api_url + 'MedicationRequest?code={}&_count={}&apikey={}'.format(
                    key,
                    10,
                    api_token
                )
                ...
                for prac in pracs:
                    ...
                    for pat in patients:
                        ...
                        for encounter in encounters:
                            ...
                            conditions = list(filter(lambda x: os.path.isdir(os.path.join(condition_path, x)) and x.startswith('condition'), os.listdir(condition_path)))
                            for condition in conditions:
                                for cond_item in generate_conditions[cond_key]:
                                    if condition == cond_item:
                                        expected_items = math.floor(random.random() * (count[1] - count[0]) + count[0])

                                        for entry in items[items_offset:items_offset + expected_items]:
                                            try:
                                                # Set patient
                                                ...
                                                entry['resource']['reasonReference'][0]['reference'] = 'urn:uuid:' + condition.replace('condition', '')
                                                # Set practitioner
                                                entry['resource']['requester']['agent']['reference'] = 'urn:uuid:' + prac.replace('practitioner', '')
                                                # Set organization
                                                entry['resource']['requester']['onBehalfOf']['reference'] = 'urn:uuid:' + organization.replace('organization', '')
                                                #entry['resource']['encounter']['reference'] = 'urn:uuid:' + encounter.replace('encounter', '')
                                            except KeyError:
                                                raise

                                            path = Path(directory_path, organization, prac, pat, encounter, 'medicationrequest{}'.format(entry['resource']['id']))

                                            if path.exists() and path.is_dir():
                                                shutil.rmtree(path)
                                            os.mkdir(path)

                                            file_path = os.path.join(path, 'medicationrequest{}.json'.format(entry['resource']['id']))
                                            del entry['search']
