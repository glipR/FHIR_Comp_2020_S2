...
def generate_conditions(condition_list, max_number_conditions, api_url, api_token, generated_conditions, directory_path):
    print("=== Generating Conditions")
    for key, display, count, medications in (
        condition_list
    ):
                ...
                for pat in patients:
                    ...
                    for encounter in encounters:
                        ...
                        for entry in items[items_offset:items_offset + expected_items]:
                            ...
                            path = Path(directory_path, organization, prac, pat, encounter, 'condition{}'.format(entry['resource']['id']))

                            if path.exists() and path.is_dir():
                                shutil.rmtree(path)
                            os.mkdir(path)

                            file_path = os.path.join(path, 'condition{}.json'.format(entry['resource']['id']))
                            generated_conditions[key].append('condition{}'.format(entry['resource']['id']))
                            del entry['search']
